  ...
  return (
    <div className={styles.root}>
      ...
      >
        <div className={styles.controlsGrid}>
          ...
          <div className={styles.sliderGroup}>
            <div className={styles.sliderMeta}>
              ...
              <span className={styles.sliderVal} style={{ color: 'var(--cyan)' }}>
                {amp.toFixed(2)}
              </span>
            </div>
            <input
          ...
          <div className={styles.sliderGroup}>
            <div className={styles.sliderMeta}>
              <span className={styles.sliderLabel}>Grid resolution</span>
              <span className={styles.sliderVal} style={{ color: 'var(--cyan)' }}>
                {res}×{res}
              </span>
            </div>
          ...
          <div className={styles.playGroup}>
            ...
            <div>
              ...
              <div className={styles.tDisplay}>t = {t.toFixed(3)}</div>
            </div>
          </div>
        ...
        <div className={styles.infoRow}>
          ...
          <div className={styles.infoChip}>
            ...
            <span className={styles.infoVal} style={{ color: 'var(--orange)' }}>
              {(-amp * Math.sin(2 * Math.PI * t)).toFixed(3)}
            </span>
          </div>
          <div className={styles.infoChip}>
            <span className={styles.infoKey}>sin(2πt)</span>
            <span className={styles.infoVal} style={{ color: 'var(--yellow)' }}>
              {Math.sin(2 * Math.PI * t).toFixed(3)}
            </span>
          </div>
          <div className={styles.infoChip}>
            <span className={styles.infoKey}>Nodes</span>
            <span className={styles.infoVal}>{(res + 1) * (res + 1)}</span>
          </div>
        </div>
      ...
      <div className={styles.canvasGrid}>
        ...
        <Card
          title="Deformed Mesh"
          badge={<Badge variant="orange">Live · Laplace solution</Badge>}
          noPadding
        >
          <div className={styles.canvasWrap}>
            <canvas ref={canvasDefRef} className={styles.canvas} />
            <div className={styles.canvasTag}>t = {t.toFixed(3)}</div>
            {/* Red legend for top boundary */}
            <div className={styles.legend}>
              <span className={styles.legendDot} />
              <span className={styles.legendLabel}>Top BC nodes</span>
            </div>
          </div>
        </Card>
      ...
      <Card
        ...
        badge={<Badge variant="cyan">v(x,t) = −A·sin(πx)·sin(2πt)</Badge>}
        noPadding
      >
      ...
      <div className={styles.note}>
        ...
        <p className={styles.noteText}>
          This is a <strong>browser-computed</strong> approximation using the
          linearised Laplace displacement model — displacement decays linearly
          from the top boundary (full amplitude) to the bottom (zero). The actual
          PINN solver computes a more accurate solution offline. Use the
          <strong> Deformation Explorer</strong> tab to view the real PINN output images.
        </p>
      </div>
